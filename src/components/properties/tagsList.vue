      ...
      style="width: 100%">
        ...
        <el-table-column
         min-width = "100px"
          ...
          <template slot-scope="scope">
            <template v-if="scope.row.edit || editId === scope.row.id">
              <el-button @click="saveTag(scope.row)" type="primary" size="small">保存</el-button>
              <el-button @click="cancelEdit()" size="small">取消</el-button>
            </template>
            <template v-else>
              <el-button @click="editTag(scope.row)" type="warning" size="small">编辑</el-button>
              <el-button @click="deleteTag(scope.row)" type="danger" size="small">删除</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    saveTag (row) {
      if (utils.isParamsEmpty(row.name, row.description)) {
        this.$message.error('标签名称和描述不能为空')
        return false
      } else if (row.id) {
        this.updateTag(row)
      } else {
